...
Simple.prototype._parse = function(str) {
  ...
  if(year.match(/^[+-][0-9]{4}$/) === null) throw new Error('Invalid Date: Malformed year');
  ...
  // If there is time
  if(str.charAt(offset) == 'T') return this._parseTime(str.substr(offset+1));

  if(str.charAt(offset) != '-') throw new Error('Invalid Date: Malformed year-month separator');

  if(end-offset < 3) throw new Error('Invalid Date: Malformed month');

  // Extract and validate month
  var month = str.substr(offset+1,2);
  if(month.match(/^(0[1-9]|1[0-2])$/) === null) throw new Error('Invalid Date: Malformed month');
  ...
  switch(this._month) {
    ...
    case 10:
    case 12:
      if(day.match(/^(0[1-9]|[1-2][0-9]|3[0-1])$/) === null) throw new Error('Invalid Date: Malformed day (31 in '+this._month+')');
      break;
    case 4:
    case 7:
    case 9:
    case 11:
      if(day.match(/^(0[1-9]|[1-2][0-9]|30)$/) === null) throw new Error('Invalid Date: Malformed day (30 in '+this._month+')');
      break;
    case 2:
      var leapyear;
      if(this._year % 4 != 0) {
      ...
      if(leapyear) {
        if(day.match(/^(0[1-9]|1[0-9]|2[0-9])$/) === null) throw new Error('Invalid Date: Malformed day (29 in '+this._month+' - leapyear)');
      } else {
        if(day.match(/^(0[1-9]|1[0-9]|2[0-8])$/) === null) throw new Error('Invalid Date: Malformed day (30 in '+this._month+')');
      }
      break;
  }
